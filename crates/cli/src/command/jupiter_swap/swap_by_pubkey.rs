...
impl JupyterSwapByPubkey {
    pub async fn run(&self) -> anyhow::Result<()> {
        ...
        let (_commitment, _payer, _rpc_enpoint) = config.read_global_config().map_err(|e| {
        ...
        // let input_token = Pubkey::from_str(&self.input_token_address).map_err(|e| {
        //     let location = std::panic::Location::caller();
        //     Error::from(format!("Error({}): {})", location, e.to_string()))
        // })?;

        // let output_token = Pubkey::from_str(&self.output_token_address).map_err(|e| {
        // let location = std::panic::Location::caller();
        // Error::from(format!("Error({}): {})", location, e.to_string()))
        // })?;
